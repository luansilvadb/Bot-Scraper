...
export class ScrapingProcessor extends WorkerHost {
    ...
    async process(job: Job<any, any, string>): Promise<any> {
        ...
        const proxyConfig = bot.proxy ? {
            server: `${bot.proxy.protocol.toLowerCase()}://${bot.proxy.host}:${bot.proxy.port}`,
            username: bot.proxy.username || undefined,
            password: bot.proxy.password || undefined,
        } : undefined;

        const context = await this.playwright.createProxyContext(proxyConfig);
        ...
        try {
            ...
            this.logger.log(`Successfully processed ${products.length} products for bot ${bot.name}`);
        } catch (error) {
            this.logger.error(`Failed to scrape for bot ${bot.name}: ${error.message}`);
            throw error;
        } finally {
            await context.close();
        }
